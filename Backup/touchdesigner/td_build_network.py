...
def buildSubmissionTemplate(project):
    ...
    set_par(fit_photo, "fit", "fill")
    set_par(fit_photo, "justifyx", "left")
    ...
    connect(final_level, composite_card, 0)
    # Bypass final_level for stability. final_level remains available for manual
    # grading, but it should not be able to blank the production output.
    connect(out1, composite_card, 0)
...
def buildPreviewSelects(project, composite):
    for index in range(MAX_PREVIEW_CARDS):
        ...
        connect(composite, transform_top, index)
